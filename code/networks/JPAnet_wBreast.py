...
class JPAnet(nn.Module):
    def __init__(self, in_channels, out_channels, is_dropput=False):
        # two DCE images and one breast_pred as inputs (C1, C0, Breast_pred)
    ...
    def forward(self, input):
        channel_input_3 = input[:, 2:3]     # breast_pred
        channel_input_1 = input[:, 0:1]     # C1
        channel_13 = torch.cat((channel_input_1, channel_input_3), dim=1)
        x1_1 = self.encoder1(channel_13)
        x2_1 = self.encoder2(x1_1)
        x3_1 = self.encoder3(x2_1)
        x4_1 = self.encoder4(x3_1)
        x5_1 = self.encoder5(x4_1)

        channel_input_2 = input[:, 1:2]     # C0
        channel_23 = torch.cat((channel_input_2, channel_input_3), dim=1)
        x1_2 = self.encoder1(channel_23)
...
if __name__ == '__main__':
    ...
    model = JPAnet(2, 2, is_dropput=False).cuda()
    x = torch.ones((1, 3, 96, 96, 96)).cuda()
    y = model(x)
    # torchsummary.summary(model, (3, 256, 64, 48))
