"""
train JPAnet on (C1, C0, BreastPred)
Exp6:
Methods -- BSegNet+CSegNet (Ours)
BSegNet -- Yes
CSegNet Input -- C1&C0&ROI
"""
...
from dataloaders.BC_Nchannels_withBreast import BC
from dataloaders.BC_Nchannels_withBreast import Crop2patchesToTensor
...
parser.add_argument("--data_root_path", type=str, default='/raid/hra/dataset/BreastCancerMRI_350/CropZero_down0.5XY/Norm/', help='dataset root path')
...
parser.add_argument("--img3_prefix", type=str, default='BreastPred', help="img prefix")
...
def train(exp_name,
    ...
    # dataset
    # training
    volume_key_list_withBreast = ['volume1', 'volume2', 'volume3']
    volume_key_list_withoutBreast = ['volume1', 'volume2']
    train_dataset = BC(train_list, data_root_path, [img1_prefix, img2_prefix, img3_prefix], label_prefix,
                       transform=transforms.Compose([RandomRotateTransform(angle_range=(-10, 10), p_per_sample=0.2,
                                                     ...
                                                     Crop2patchesToTensor(patch_size,
                                                                          volume_key_list=volume_key_list_withBreast)]))
    val_dataset = BC(val_list, data_root_path, [img1_prefix, img2_prefix, img3_prefix], label_prefix)

    # dataloader
    def worker_init_fn(worker_id):
        random.seed(seed1 + worker_id)
    train_dataloader = DataLoader(train_dataset, batch_size=None, shuffle=True, num_workers=16,
                                  pin_memory=True, worker_init_fn=worker_init_fn)
    val_dataloader = DataLoader(val_dataset, batch_size=None, shuffle=False)

    """ model, optimizer, loss """
    model = JPAnet(in_channels=2, out_channels=2, is_dropput=False).cuda()
    ...
    for epoch in range(max_epoch):
        ...
        for i_batch, sampled_batch in enumerate(train_dataloader):
            ...
            if n_total_iter % 100 == 0:
                ...
                image = volume_batch3[1, 0:1, :, :, ::2].permute(3, 0, 2, 1).repeat(1, 3, 1, 1)
                grid_image = make_grid(image, 8, normalize=True)
                writer.add_image('Image3', grid_image, n_total_iter)
            ...
            if n_total_iter % n_val_iter == 0:
                ...
                for val_sample in tqdm(val_dataloader):
                    ...
                    val_input3 = val_sample['volume3'].cpu().detach().numpy()
                    val_label = val_sample['label'].cpu().detach().numpy()

                    prediction, score_map = test_single_case(model, [val_input1, val_input2, val_input3],
