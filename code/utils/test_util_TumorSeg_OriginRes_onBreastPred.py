...
def test_all_case_Nchannel_with_image_processing(net_tumor, net_breast, test_list, data_root_path,
                                                 img_prefix_list, label_prefix,
                                                 exp_name, num_classes, patch_stride_breast, patch_stride_tumor,
    ...
    for index, image_id in enumerate(test_list):
        ...
        breast_input = nib.load(os.path.join(data_root_path, 'DCE-C0', 'DCE-C0' + '_' + image_id)).get_fdata()
        ...
        image_pre_processed_forBreastSeg = ImageProcessingBreastSegInstance.pre_func(breast_input, affine)
        breast, _ = test_single_case_Nchannel(net_breast, [image_pre_processed_forBreastSeg],
                                           patch_stride_breast["stride_x"], patch_stride_breast["stride_y"],
                                           patch_stride_breast["stride_z"], patch_stride_breast["patch_size"], num_classes)
        breast = ImageProcessingBreastSegInstance.post_func(breast, fill_hole=True, remove_but_largest=True)

        img_list.append(breast)
        image_pre_processed_list = ImageProcessingTumorSegInstance.pre_func(img_list, affine)
        prediction, score_map = test_single_case_Nchannel(net_tumor, image_pre_processed_list,
                                                          patch_stride_tumor["stride_x"], patch_stride_tumor["stride_y"],
                                                          patch_stride_tumor["stride_z"], patch_stride_tumor["patch_size"],
                                                          num_classes)
        prediction = ImageProcessingTumorSegInstance.post_func(prediction)

        if use_mirror_ensemble:
            prediction = mirror_ensemble(image_pre_processed_list, net_tumor,
                                         patch_stride_tumor["stride_x"], patch_stride_tumor["stride_y"],
                                         patch_stride_tumor["stride_z"], patch_stride_tumor["patch_size"],
        ...
        if save_result:
            nib.save(nib.Nifti1Image(breast.astype(np.uint8), affine), test_save_path + "/%sbreast.nii.gz" % name)
