"""
train Baseline Model (ResUnet) on C1
Exp1:
...
from dataloaders.BC import BC
from dataloaders.BC import Crop2patchesToTensor
from dataloaders.data_augmentation import *
from networks.BaselineModel import ResUnet

# 参数设置
parser = argparse.ArgumentParser()
parser.add_argument("--exp_name", type=str, default='Baseline_CropZeroDown0.5XY_C1_DatalistSiemens',
...
parser.add_argument("--img_prefix", type=str, default='DCE-C1', help="img prefix")
...
def train(exp_name,
    ...
    # dataset
    # training
    train_dataset = BC(train_list, data_root_path, img_prefix, label_prefix,
    ...
    val_dataset = BC(val_list, data_root_path, img_prefix, label_prefix)
    ...
    model = ResUnet(in_channels=1, out_channels=2, is_dropput=False).cuda()
    ...
    for epoch in range(max_epoch):
        ...
        for i_batch, sampled_batch in enumerate(train_dataloader):
            ...
            # run model
            seg_output_list = model(volume_batch)
            ...
            if n_total_iter % 100 == 0:
                ...
                image = volume_batch[1, 0:1, :, :, ::2].permute(3, 0, 2, 1).repeat(1, 3, 1, 1)
            ...
            if n_total_iter % n_val_iter == 0:
                ...
                for val_sample in tqdm(val_dataloader):
                    val_input = val_sample['volume'].cpu().detach().numpy()
                    val_label = val_sample['label'].cpu().detach().numpy()
                    prediction, score_map = test_single_case(model, val_input,
